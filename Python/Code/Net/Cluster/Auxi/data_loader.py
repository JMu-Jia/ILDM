...
from Code.Net.Cluster.Auxi.uea import subsample, interpolate_missing, Normalizer
...
class UEAloader(Dataset):
    ...
    def __init__(self, root_path, lab_rep, file_list=None, limit_size=None, flag=None):
        self.root_path = root_path
        self.lab_rep = lab_rep  # 标签替换
        ...
        self.feature_df = normalizer.normalize(self.feature_df) # 归一化
    ...
    def load_all(self, root_path, file_list=None, flag=None):
        ...
        if file_list is None:   # 导入文件夹下路径
            data_paths = glob.glob(os.path.join(root_path, '*'))  # list of all paths
        else:
            data_paths = [os.path.join(root_path, p) for p in file_list]
        if len(data_paths) == 0:    # 没有文件则报错
            raise Exception('No files found using: {}'.format(os.path.join(root_path, '*')))
        if flag is not None:    # 选择训练或测试的数据集
    ...
    def load_single(self, filepath):    # 读取ts文件
