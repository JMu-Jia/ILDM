...
class Attention(nn.Module):
    ...
    def get_spatial_attention(self, x):
        spatial_attention = self.spatial_fc(x).view(x.size(0), 1, 1, 1, -1)
...
class ODConv1d(nn.Module):
    ...
    def _forward_impl_common(self, x):
        ...
        batch_size, in_planes, Sequence = x.size()

        # channel_attention为 [N, C_in, 1]
        x = x * channel_attention
        # 把输入特征由 [N, C_in, S] 转化为 [1, N*C_in, S]
        x = x.reshape(1, -1, Sequence)

        # weight [K, C_out, C_in/groups, 3]--->[1, k_num, C_out, C_in/groups, k_size]
        # spatial_attention                    [N, 1, 1, 1, k_size]
        # kernel_attention                     [N, k_num, 1, 1, 1]
        aggregate_weight = spatial_attention * kernel_attention * self.weight.unsqueeze(dim=0)
        # aggregate_weight = kernel_attention * self.weight.unsqueeze(dim=0)
        # aggregate_weight [N, k_num, C_out, C_in/groups, k_size]
        # --->[N, C_out, C_in/groups, k_size]
        # --->[N*C_out, C_in/groups, k_size]
        aggregate_weight = torch.sum(aggregate_weight, dim=1).view(
            [-1, self.in_planes // self.groups, self.kernel_size])
        # [N*C_out, C_in/groups, k_size] * [1, N*C_in, S]
